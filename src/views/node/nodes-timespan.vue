<template>
  <div class="app-container">
    ...
    <b-row v-if="WorkNodesFilters.eWorkNodeNo && WorkNodesFilters.eWorkNodeNo !== ''">
      <b-col v-for="(node, index) in nodesYieldRate" :key="index" cols="12" sm="3" class="mb-4">
        <b-card class="app-card">
          <YieldRatePieChart :data="node.chartData" :title="node.nodeNo" :time="node.nodeTitle" />
        </b-card>
      </b-col>
    </b-row>
  </div>
...
import { GetLoopProgressRate, GetLoopLastWorkNode } from '@/api/ProgressRate'
import { nodeYieldRateToPieChart } from '@/utils/nodeYieldRate'
import DateTimeRange from '@/components/DateTimeRange'
import YieldRatePieChart from '@/components/charts/PieChart/YieldRate.vue'
...
export default {
  ...
  computed: {
    ...
    nodesYieldRate() {
      const result = nodeYieldRateToPieChart(this.nodeData, this.WorkNodesFilters.eWorkNodeNo)
      return result
    },
    LangNo() {
